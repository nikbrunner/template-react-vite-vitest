# Review: Todo List with Filters - 2026-01-26
...
<li className="TodoItem" onClick={() => props.onComplete(props.todo)}>
  {/* ... */}
  <button onClick={() => props.onDelete(props.todo)}>⌫</button>
</li>
```
...
**Problem:** This doesn't use any React hooks (useState, useEffect, etc.). It's just a pure function. Naming it `useFilter` is misleading.
...
3. Add the visual completed state - quick CSS win

4. Fix the event propagation bug - common interview gotcha
